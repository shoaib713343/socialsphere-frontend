...
const ProfilePage = () => {
  ...
  // Use the username from the URL, or fallback to the logged-in user's username
  ...
  const fetchProfileAndPosts = useCallback(async () => {
    ...
    try {
      // --- THIS IS THE FIX ---
      // We will now make the calls sequentially to avoid race conditions.

      // Step 1: Fetch the user's profile data using their username.
      const profileRes = await api.get(`/users/${effectiveUsername}`);
      const userProfile: UserProfile = profileRes.data.data;
      setProfile(userProfile);

      // Step 2: Once we have the profile, use the SAME USERNAME to fetch their posts.
      if (userProfile?.username) {
        const postsRes = await api.get(`/posts/user/${userProfile.username}`);
  ...
  if (error || !profile) {
    return (
      <div className="text-center mt-10">
        ...
        <Link to="/" className="mt-4 inline-block px-4 py-2 text-white bg-indigo-600 rounded-md">
          Back to Home
        </Link>
      </div>
    );
        ...
        <div className="bg-white shadow-md rounded-lg p-6 mb-8 flex items-center">
          <div className="relative">
            <img 
              ...
              src={ (isOwnProfile ? currentUser?.profilePicture : profile?.profilePicture) || `https://i.pravatar.cc/150?u=${profile._id}` } 
              alt={profile.username} 
            />
            {isOwnProfile && (
              <button 
                onClick={() => setIsAvatarModalOpen(true)} 
                className="absolute bottom-0 right-6 bg-white p-1.5 rounded-full shadow-md hover:bg-gray-100"
              >
